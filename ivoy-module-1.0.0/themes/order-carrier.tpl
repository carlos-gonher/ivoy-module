...
*}
{if $is_logged}
    {assign var='customer_id' value=Context::getContext()->customer->id}
{else}
    {assign var='customer_id' value=0}
{/if}
{assign var='store_data' value=Store::getData(Context::getContext()->shop->id)}
{if !$opc}
		...
		{else}
					...
					{foreach $delivery_option_list as $id_address => $option_list}
						...
						<div class="delivery_options">
							{foreach $option_list as $key => $option}
                                                                {if 'ivoy' == {$option['carrier_name']|lower}} {assign var="ivoy" value="1"} {else} {assign var="ivoy" value="0"} {/if}
								<div class="delivery_option {if ($option@index % 2)}alternate_{/if}item">
									<div>
										<table class="resume table table-bordered{if !$option.unique_carrier} not-displayable{/if}">
											<tr>
												<td class="delivery_option_radio">
                                                                                                    <input id="delivery_option_{$id_address|intval}_{$option@index}" class="delivery_option_radio" type="radio" name="delivery_option[{$id_address|intval}]" data-key="{$key}" data-id_address="{$id_address|intval}" data-ivoy="{$ivoy}" value="{$key}"{if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key} checked="checked"{/if} />
												</td>
												<td class="hidden-xs delivery_option_logo">
												...
												<td>
                                                                                                        {if $ivoy}
                                                                                                            <div id="theme-info" 
                                                                                                                 data-store-name="{$store_data['name']}" 
                                                                                                                 data-store-id="{Context::getContext()->shop->id}" 
                                                                                                                 data-customer-id="{$customer_id}" 
                                                                                                                 data-cart-id="{Context::getContext()->cart->id}" 
                                                                                                                 data-carrier="{$key}" 
                                                                                                                 data-uri="{$tpl_uri}" 
                                                                                                                 data-base="{$base_dir_ssl}" 
                                                                                                                 data-lat="{$store_data['latitude']}" 
                                                                                                                 data-lng="{$store_data['longitude']}" 
                                                                                                                 data-ivoy-module="{$link->getModuleLink('ivoydelivery', 'validate')}">
                                                                                                            </div>
                                                                                                            <div id="delivery-ivoy-window" class="delivery-ivoy-window" style="display:none;">
                                                                                                                ¿A qu&eacute; lugar quieres enviar?
                                                                                                                <br>
                                                                                                                <input type="button" id="zipcode_search" class="delivery-ivoy-button" value="Mi direcci&oacute;n">
                                                                                                                <input type="button" id="address_search" class="delivery-ivoy-button" value="Buscar direcci&oacute;n">
                                                                                                            </div>
                                                                                                            
                                                                                                            <div id="delivery-ivoy-section">
                                                                                                                {if $option.unique_carrier}
                                                                                                                        ...
                                                                                                                        {/if}
                                                                                                                {/if}
                                                                                                                <div class="col-sm-6 col-md-4 col-lg-4">¿Qu&eacute; es esto?</div>
                                                                                                                <div class="col-sm-6 col-md-4 col-lg-4">¿Horarios?</div>                                                                                                            
                                                                                                            </div>                                                                                                            
                                                                                                        {/if}
													...
													{/if*}
                                                                                                        ...
                                                                                                        {if $key != 91}
                                                                                                            <br><strong class="col-sm-12 col-md-12 col-lg-12" style="color:red;">No realizamos envíos "Ocurre".</strong>
                                                                                                        {/if}
                                                                                                    
												</td>
												<td class="delivery_option_price">
													<div class="delivery_option_price" {if $ivoy}id="ivoy_option_price" data-addr-selected="0"{/if}>
											...
											<table class="delivery_option_carrier{if isset($delivery_option[$id_address]) && $delivery_option[$id_address] == $key} selected{/if} resume table table-bordered{if $option.unique_carrier} not-displayable{/if}">
												<tr>
													...
													<td class="delivery_option_logo{if $first.product_list[0].carrier_list[0] eq 0} not-displayable{/if}">
														...
														{/if}
														{if $key != 91}
														<br><strong style="color:red;">No realizamos envíos "Ocurre".</strong>
														{/if}
													</td>
			...
			{if $conditions AND $cms_id}
				...
				<p class="checkbox">
					...
					<a style="color:red;" href="{$link_conditions|escape:'html':'UTF-8'}" class="iframe" rel="nofollow">{l s='(Read the Terms of Service)'}</a>
				</p>
